...
let connected=false,busy=false,messages=[],lastFingerprint='',noticeTimer,refreshPressTimer,refreshHintTimer,refreshHideTimer,skipRefreshClick=false;
let notificationEnabled=false,accent='default',cadenceMode='foreground';
let draftFiles=[],replyId='',fileBusy=false,lastChecked=0,statusOverride='';
const devlogEntries=[{version:'1.0.2',date:'September 11, 2026',title:'Attachments and everyday reliability',summary:'Send files, find past messages, and recover failed sends.',notes:['Attach up to 8 files totaling 12 MB, with encrypted drafts.','Search, copy, reply to a selected message, and dictate editable text.','Load older history, see the last sync time, and recover definite send failures.','First-reply notification fix and tap-to-refresh lightning logo.']},{version:'1.0.1',date:'September 11, 2026',title:'First release',summary:'Android email chat with light and dark themes.',notes:['Send and receive through Gmail while keeping history and drafts encrypted on your phone.','Elastic scrolling, tap feedback, and a button to jump to new replies.','Notification controls and instructions for muting Instinct emails in Gmail.']}];
...
function updateComposer(){const input=$('draft');input.style.height='auto';input.style.height=Math.min(input.scrollHeight,140)+'px';$('sendButton').disabled=!connected||busy||fileBusy||(!input.value.trim()&&!draftFiles.length);input.disabled=busy;$('attachButton').disabled=busy||fileBusy;$('cancelReply').disabled=busy;$('dictateButton').disabled=busy;document.querySelectorAll('#fileChips button').forEach(button=>button.disabled=busy||fileBusy);$('conversationTools').hidden=!connected;}
...
function render(data){
 ...
 for(const message of data){
  ...
  if(message.status==='received'||message.status==='sent'){const reply=document.createElement('button');reply.textContent='Reply';reply.onclick=()=>{if(busy)return;native('reply',message.id);$('draft').focus();};meta.append(reply);}
...
$('sendButton').onclick=()=>{const text=$('draft').value.trim();if((!text&&!draftFiles.length)||busy||fileBusy||!connected)return;busy=true;$('sendButton').classList.add('sending');updateComposer();native('send',text);};
